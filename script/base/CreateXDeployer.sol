...
/**
 * @title Official CreateX interface
 */
interface ICreateX {
...
abstract contract CreateXDeployer is Script {
    ...
    function computeCreate3Address(bytes32 salt, address deployer) internal view returns (address predicted) {
        bytes32 guardedSalt;
        if (address(bytes20(salt)) == deployer && bytes1(salt[20]) == hex"00") {
            guardedSalt = keccak256(abi.encodePacked(bytes32(uint256(uint160(deployer))), salt));
        } else if (address(bytes20(salt)) == deployer && bytes1(salt[20]) == hex"01") {
    ...
    {
        address deployer = address(bytes20(salt));
        address expected = computeCreate3Address(salt, deployer);
        ...
        console.log(identifier, "-", deployed);
    }
