...
async function main() {
  ...
  const poolToken = await upgrades.deployProxy(TapETH, [deployer.address], {
    timeout: 600000,
  });
  ...
  const stETHSwap = await upgrades.deployProxy(
    StableAsset,
    [
      [wETHAddress, stETHAddress],
      [PRECISION, PRECISION],
  ...
  application.updatePool(stETHSwap.address, true);
  application.updatePool(rETHSwap.address, true);
